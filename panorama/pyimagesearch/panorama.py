...
def XYZtoRC(dataset_type, hits, img, cam_id):
    if dataset_type == 'nclt':
        ...
        with open("/mnt/workspace/workgroup/xxc/datasets/Oxford/image_meta.pkl", 'rb') as handle:
            image_meta = pickle.load(handle)
...
def spherer_projection(dataset_type, imgs, f) :
    R = f
    rows = imgs[0].shape[0]
    cols = imgs[0].shape[1]
    ...
    for cam_id in range(len(imgs)):
        ...
        if dataset_type == 'nclt':
            ...
            panoramic[valid_y, valid_x, 0] = rgb[...,2]
            panoramic[valid_y, valid_x, 1] = rgb[...,1]
            panoramic[valid_y, valid_x, 2] = rgb[...,0]
...
class Stitcher:
    ...
    def stitch(self, images, dataset_type):
        ...
        focal = intrins[3][1,1]
        result, cv_img = spherer_projection(dataset_type, images, focal)
