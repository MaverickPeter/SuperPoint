...
if __name__ == '__main__':
  ...
  for ndx in tqdm(range(len(train_data))):
    ...
    if opt.dataset_type == 'nclt':
      ...
      img_filename = os.path.join(opt.dataset_root, train_data[ndx].filepaths[5])
    ...
    if opt.use_sift:
      kps, desc = sift.detectAndCompute(sift_img,None)
      if desc is not None:
        heatmap = None
      ...
      kps, desc = orb.detectAndCompute(sift_img,None)
      heatmap = None
      pts = cv2.KeyPoint_convert(kps)
