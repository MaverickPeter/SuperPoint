...
if __name__ == '__main__':
  ...
  for ndx, e in tqdm(enumerate(map_set)):
    ...
    if opt.dataset_type == 'nclt':
      ...
      img_filename = os.path.join(opt.dataset_root, e.filepaths[5])
    ...
    if opt.use_sift:
      ...
      if pts_ascend.shape[0] > 300:
        ...
        desc = desc.transpose()
      # else:
      #   pts_new = np.zeros((300, 3))
      #   pts_new[:pts_ascend.shape[0],2] = pts_ascend
      #   desc_new = np.zeros((300, 256))
      #   if desc is not None:
      #     desc_new[:desc.shape[1],...] = desc.transpose()
      #   desc = desc_new
      #   pts = pts_new
